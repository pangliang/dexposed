    ...
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
        ...
        if (args.size() > 0 || (target_sdk_version > 0 && target_sdk_version <= 22)) {
            ...
            for (size_t i = 0; i < args.size(); ++i) {
                ...
                switch(shorty[i + 1]){
                    case 'L':
                        jv.SetL(soa.Decode<mirror::Object *>(args[i].l));
                        break;
                    case 'D':
                    case 'J':
                        jv.SetJ(args[i].j);
...
//                objectArray->Set<false>(i,val);
                objectArray->SetWithoutChecks<false,false,kVerifyNone>(i,val);
            }
        }
...
//        art_method->SetAccessFlags((art_method->GetAccessFlags() & ~kAccNative));
    ...
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
        ...
        uint32_t shorty_len;
        const char* shorty = art_method->GetShorty(&shorty_len);
