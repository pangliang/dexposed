...
class MANAGED ArtMethod FINAL : public Object {
 ...
 public:
  ...
  // Short cuts to declaring_class_->dex_cache_ member for fast compiled code access.
  HeapReference<ObjectArray<String>> dex_cache_strings_; //20

  // Method dispatch from the interpreter invokes this pointer which may cause a bridge into
  // compiled code.
  uint64_t entry_point_from_interpreter_; //24

  // Pointer to JNI function registered to this method, or a function to resolve the JNI function.
  uint64_t entry_point_from_jni_;  //24+8=32
  ...
  uint64_t entry_point_from_quick_compiled_code_; //40
  ...
  uint32_t access_flags_;  //56

  /* Dex file fields. The defining dex file is available via declaring_class_->dex_cache_ */

  // Offset to the CodeItem.
  uint32_t dex_code_item_offset_; //60

  // Index into method_ids of the dex file associated with this method.
  uint32_t dex_method_index_;  //64
